...
block_icmp(){
    output "ICMP Paketlerini Engelle?"
    output "Bir sistem kullanmıyorsanız [1]'i, aksi halde [2]'yi seçmelisiniz.."
    output "[1] Evet."
    output "[2] Hayır."
    read icmp
    case $icmp in
        1 ) /sbin/iptables -t mangle -A PREROUTING -p icmp -j DROP
            ...
            ;;
        2 ) output "Kural geçiliyor..."
            ;;
        * ) output "Geçerli bir seçim girmediniz."
            block_icmp
    esac    
}

block_bruteforce(){
    output "SSH Bruteforce Engeli?"
    output "SSH Bruteforce'u engellemek için SADECE Fail2ban veya başka bir yöntem kullanmıyorsanız [1]'i seçmelisiniz. Aksi halde [2]'yi seçin."
    output "[1] Evet."
    output "[2] Hayır."
    read bruteforce
    case $bruteforce in
...
javapipe_kernel(){
    output "JavaPipe'ın kernel yapılandırmaları uygulansın mı? (https://javapipe.com/blog/iptables-ddos-protection/)"
    output "[1] Evet. (Önerilen CentOS 8/7)"
    output "[2] Hayır."
    read javapipe
    case $javapipe in
...
EOF
            sysctl -p 
            ;;
        2)  output "Javapipe değişiklikleri uygulanmadı."
            ;;
        * ) output "Geçerli bir seçim girmediniz."
            javapipe_kernel
    esac    
...
output "VADIWALL SCRIPT"
output "© 2021 SUNUCUVADISI.HOST <sunucuvadisii@gmail.com>."
output "Yapımcı: Ada Gürsoy"
output ""
...
if [ "$lsb_dist" =  "ubuntu" ] || [ "$lsb_dist" =  "debian" ]; then
     ...
     output "Desteklenmeyen işletim sistemi. Bu komut dosyası yalnızca Fedora, RHEL, CentOS, Ubuntu ve Debian'ı destekler."
fi 
...
block_bruteforce
javapipe_kernel
output "VadiWall kuruldu, sunucunuzu yeniden başlatıp oyun sunucunuzu aktif edebilirsiniz."
